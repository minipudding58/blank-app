...
if not st.session_state.user_id:
    ...
    user_input = st.text_input("닉네임 입력 (예: 치이카와)", placeholder="닉네임을 입력해주세요.")
...
st.markdown("""
    ...
    /* 일반 버튼: 높이 통일 및 투명 배경 */
    div.stButton > button {
        height: 38px !important; background-color: transparent !important; border: 1px solid #ccc !important;
    }
    /* 삭제/로그아웃 버튼 전용 스타일 */
...
with st.sidebar:
    ...
    st.write("---")
    st.warning("⚠️ 주의: 아래 버튼을 누르면 이 닉네임의 모든 데이터가 서버에서 완전히 삭제됩니다.")
    if st.button("🔥 내 기록 전체 삭제 및 폐쇄", use_container_width=True):
        if os.path.exists(USER_DATA_FILE):
            os.remove(USER_DATA_FILE) # 파일 물리적 삭제
            st.query_params.clear()
            st.session_state.user_id = ""
            st.session_state.collection = []
            st.session_state.wishlist = []
