...
# 🔍 5. 검색 (버튼 옆 공백 적용)
# ==========================================
st.markdown("<span class='header-label'>🔍 도서 검색</span>", unsafe_allow_html=True)
...
# ==========================================
# 📚 6. 메인 탭 (위시리스트 수평 정렬 적용)
# ==========================================
tab_lib, tab_wish = st.tabs(["📚 내 서재", "🩵 위시리스트"])

with tab_lib:
    is_edit = st.toggle("편집 모드", key="edit_toggle")
    if st.session_state.collection:
            ...
            with lib_cols[i % 4]:
                ...
                if is_edit:
                    ...
                    if st.button("저장", key=f"esv_{i}"):
                        st.session_state.collection[i]["genre"] = new_g; commit_changes(); st.rerun()
