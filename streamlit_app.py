...
# --- 📊 상단 섹션 (달력 축소 및 비율 조정) ---
top_c1, top_c2 = st.columns([1, 1.5]) # 달력을 대폭 축소 (1.5 비율)
with top_c1:
    # 누적 독서 (배경색 없음, ✨ 추가)
...
with top_c2:
    # ✅ 2번 해결: 달력 컨트롤 (연월 제목과 같은 열의 양쪽 끝으로 이동)
    ...
    # 달력 높이를 축소하여 콤팩트하게 표시 (cite: KakaoTalk_20260322_235849172.jpg)
    for week in cal:
        cols = st.columns(7)
...
# ✅ 3번 해결: 목록과 위시리스트의 책 시작 높이 맞춤 (공백 활용)
with l_col:
...
with r_col:
    st.markdown("<p style='font-size:24px; font-weight:bold;'>🩵 위시리스트</p>", unsafe_allow_html=True)
    if st.session_state.wishlist:
        # ✅ 3번 해결: 목록과 시작 높이 맞춤 (공백 추가)
        st.write("") # 공백 추가
        st.write("") # 공백 추가
        wcols = st.columns(3)
