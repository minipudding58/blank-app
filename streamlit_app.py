                ...
                for itm in d.get("collection", []):
                    ...
                    if r.status_code == 200:
                        st.session_state.collection.append({
                            ...
                            "genre": itm.get("genre", "미지정") # 장르 정보 추가
                        })
        except: pass
...
with t_col1:
    ...
    if st.session_state.collection:
        ...
        # 보기 좋게 한 줄에 표시
        genre_str = ", ".join([f"{g} {c}권" for g, c in genre_counts.items()])
        st.write(f"**예시) {genre_str}**") # 예시 문구 포함하여 표시
    else:
    ...
    for week in cal:
        ...
        for i, day in enumerate(week):
            if day != 0:
                ...
                with cols[i]:
                    st.write(f"**{day}**")
                    # ✅ 해당 날짜에 읽은 책이 있으면 이모지를 날짜 바로 밑에 표시
...
if query:
    # 예시 장르 목록 (나중에 알라딘 API 등에서 가져오도록 확장 가능)
    example_genres = ["만화", "에세이", "소설", "로맨스", "인문학"]
        ...
        if imgs:
            ...
            for i, url in enumerate(imgs[:4]):
                with scols[i]:
                    ...
                    # 장르 선택 드롭다운 추가
                    selected_genre = st.selectbox("장르", example_genres, key=f"genre_{i}")
                    # 읽은 기간 설정
    ...
    except Exception as e: st.error(f"검색 에러")
    ...
    if st.session_state.collection:
        ...
        for idx, itm in enumerate(st.session_state.collection):
            with dcols[idx % 4]:
                ...
                st.caption(f"장르: {itm.get('genre', '미지정')}") # 장르 표시
                # 날짜 직접 수정 기능
