...
# --- 🎨 스타일 (탭 디자인 및 수평 정렬 해결) ---
st.markdown(f"""
    <style>
    .block-container {{ padding-top: 1.5rem !important; }}
    
    /* 1. 탭(Tab) 디자인: 폰트 키우고 볼드 처리, 하단 바 강조 */
    button[data-baseweb="tab"] {{
        font-size: 24px !important;
        font-weight: 800 !important;
        color: #31333F !important; /* 선택 시 색상 변화 삭제 (검정 유지) */
...
# --- [하단] 탭 영역 (탭 디자인 적용) ---
tab_lib, tab_wish = st.tabs(["✅ 읽은 책", "🩵 위시리스트"])
            ...
            with dcols[idx % 3]:
                ...
                with b_edit_cols[1]:
                    if del_m:
                        ...
                        if st.button("❌", key=f"dc_{idx}", use_container_width=True):
